...
@section('title')
    {{ __('crud.general.user') }} {{ __('crud.general.login') }}
@endsection
...
@section('content')
    <section class="relative py-20" x-data="alpineWalletVars()" x-cloak>
        ...
        <div class="absolute top-0 left-0 lg:bottom-0 h-128 lg:h-auto w-full lg:w-8/12 bg-cover bg-no-repeat bg-gray-800 opacity-50" @if(array_key_exists('login_user_bg_img', $settings)) style="background-image: url('{{ asset('storage/'.$settings['login_user_bg_img']) }}');" @endif></div>
        <div class="relative container px-4 mx-auto">
            <div class="flex flex-wrap items-center -mx-4">
                <div class="w-full lg:w-1/2 px-4 mb-12 lg:mb-0">
                    <div class="max-w-lg">
                        <h2 class="mb-1 dark:text-gray-100 text-4xl font-semibold font-heading">{{ array_key_exists('login_user_heading', $settings) ? $settings['login_user_heading'] : 'Dummy Heading Here' }}</h2>
                        <p class="text-xl dark:text-gray-200 text-gray-800">{{ array_key_exists('login_user_desc', $settings) ? $settings['login_user_desc'] : '' }}</p>
                    </div>
                </div>
                <div class="w-full lg:w-1/2 px-4">
                    <div class="lg:max-w-md p-6 lg:px-10 lg:py-12 bg-white dark:bg-gray-900 text-center border dark:border-gray-800 rounded-xl" x-show.transition.in="!forgotPassword">
                        <form action="{{ route('user.login') }}" method="post">
                            <span class="inline-block mb-1 text-xs text-blue-400 dark:text-gray-400 font-semibold">Sign In</span>
                            <h3 class="mb-8 text-3xl font-semibold font-heading dark:text-blue-100">Join our community</h3>
                            ...
                            <a @click="toggleForgotPassword" class="block pb-6 float-right text-sm font-semibold hover:underline dark:text-gray-300" href="javascript:void(0)">Forgot password?</a>

                            @csrf

                            <button class="w-full inline-block py-4 text-sm text-white font-medium leading-normal bg-red-400 hover:bg-red-300 rounded transition duration-200">{{ __('crud.general.login') }}</button>
                            ...
                            {{-- Registration --}}
                            <a class="flex items-center justify-center mb-4 py-4 bg-indigo-800 hover:bg-indigo-900 rounded" href="{{ route('user.register') }}">
