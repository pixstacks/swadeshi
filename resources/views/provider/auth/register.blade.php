...
@section('title')
    {{ __('crud.general.provider') }} {{ __('crud.general.registration') }}
@endsection

@section('content')
    <section class="relative py-20">
        <img class="hidden lg:block absolute top-0 left-0 mt-16 z-10" src="zeus-assets/icons/dots/blue-dot-left-bars.svg" alt="">
        <div class="absolute top-0 left-0 lg:bottom-0 h-128 lg:h-auto w-full lg:w-8/12 bg-cover bg-no-repeat bg-gray-800 opacity-50" @if(array_key_exists('register_provider_bg_img', $settings)) style="background-image: url('{{ asset('storage/'.$settings['register_provider_bg_img']) }}');" @endif></div>
        <div class="relative container px-4 mx-auto">
            <div class="flex flex-wrap items-center -mx-4">
                <div class="w-full lg:w-1/2 px-4 mb-12 lg:mb-0">
                    <div class="max-w-lg">
                        <h2 class="mb-1 text-4xl dark:text-gray-100 font-semibold font-heading">{{ array_key_exists('register_provider_heading', $settings) ? $settings['register_provider_heading'] : 'Dummy Heading Here' }}</h2>
                        <p class="text-xl dark:text-gray-200 text-gray-800">{{ array_key_exists('register_provider_desc', $settings) ? $settings['register_provider_desc'] : '' }}</p>
                    </div>
                </div>
                <div class="w-full lg:w-1/2 px-4">
                    <div class="lg:max-w-lg p-6 lg:px-10 lg:py-12 bg-white dark:bg-gray-900 dark:border-gray-800 text-center border rounded-xl">
                        <form action="{{ route('provider.register') }}" method="post">
                            ...
                            {{-- Telephone --}}
                            <div class="relative flex flex-wrap mb-6">
                                <input type="tel" class="bg-gray-50 outline-none dark:bg-gray-700 dark:text-gray-200 relative mb-2 md:mb-0 w-full py-4 pl-4 text-sm rounded inptFielsd" id="phone" value="{{ old('mobile', '') }}" placeholder="123456789" />
