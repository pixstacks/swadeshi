                ...
                <div class="flex justify-center items-center w-full" style="height: -webkit-fill-available">
                    ...
                    >
                        <div class="w-full md:pt-0 pt-5 dark:text-gray-200">
                            ...
                            <hr>
                            <span class="text-xl">
                                {{ currency(auth()->user('provider')->wallet_balance) }}
                            </span>
                        </div>
                    </div>
                    <div x-show.transition.in="!showWallet"
                    ...
                    >
                        <div class="w-full">
                            <x-form method="post" :action="route('provider.addToWallet')" onkeydown="return event.key != 'Enter';" onsubmit="return validateForm()">
    ...
    @push('endScripts')
        <script>
            ...
            function validateForm() {
                ...
                console.log(alpineVars.newCard);
                ...
                function handleServerResponse(responseJson) {
                    try {
                        ...
                        .then(function(result) {
                            if (result.error) {
                                location.replace("{{ route('provider.wallet') }}");
                            }
                            else {
                                if(result.paymentIntent.status == 'succeeded' || result.paymentIntent.status == 'canceled') {
                                    ...
                                    if (responseJson.requiresAction) {
                                        ...
                                        stripe.handleCardAction(
                                            ...
                                            if (result.error) {
                                                location.replace("{{ route('provider.stripe.paymentFailed', session('intentId')) }}");
                                            } else {
                                                location.replace("{{ route('provider.stripe.paymentSuccessful', session('intentId')) }}");
                                            }
                                        });
                                    }
                                }
