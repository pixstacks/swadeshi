...
class RegisterController extends Controller
{
    ...
    /**
     * Where to redirect provider after registration.
     * 
     * @var string
     */
    ...
    {
        $results = Setting::where('key', 'like', 'register_provider_%')->get();
        $settings = [];
        foreach($results as $result) {
            $settings[$result['key']] = $result['value'];
        }
        return view('provider.auth.register', compact('settings'));
    }
    ...
    public function register(Request $request)
    {
        ...
        if ($response = $this->registered($request, $provider)) {
            return $response;
        }
